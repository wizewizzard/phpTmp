...
{foreach $partners as $id => $data}
    ...
    {if $even == 1 and $count == 7}
        ...
        <div class="odd">
    {/if}
    <a href="{if isset($version)}/{$version}{/if}/partners/view/{$id}" {if $data.logo != ''} data-logo="/upload/partnerPhotos/{$data.logo}"{/if} {if $data.photo != ''} data-replacement="{$data.photo}"{/if} class="hexagon partner" {if $data.logo != ''}style="background-image: url(/upload/partnerPhotos/{$data.logo});"{/if}>
